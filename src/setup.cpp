...
#define DATA_PREFIX "/vol/content"
...
void st_video_setup(void)
{
  ...
  if (SDL_Init(SDL_INIT_VIDEO) < 0)
    {
	  char err[256];
	  printf("Error: I could not initialize video!\n"
  ...
  if (use_fullscreen)
    {
      ...
      if (window == NULL)
        {
		  char err[256];
		  printf("Warning: I could not set up fullscreen video for "
                  "640x480 mode.\n"
                  "The Simple DirectMedia error that occured was:\n"
                  "%s\n\n", SDL_GetError());
  ...
  else
    {
      ...
      if (window == NULL)
        {
		  char err[256];
		  printf("Error: I could not set up video for 640x480 mode.\n"
                  "The Simple DirectMedia error that occured was:\n"
                  "%s\n\n", SDL_GetError());
		  SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_ERROR, "Error", err, window);
          exit(1);
  ...
  if (use_fullscreen)
  ...
  else
    {
      ...
      if (window == NULL)
        {
		  char err[256];
		  printf("Error: I could not set up video for 640x480 mode in OpenGL.\n"
                  "The Simple DirectMedia error that occured was:\n"
                  "%s\n\n", SDL_GetError());
		  SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_ERROR, "Error", err, window);
          exit(1);
      ...
      if (SDL_Init(SDL_INIT_AUDIO) < 0)
        {
          ...
          if (use_sound || use_music)
            {
			  char err[256];
			  printf("Warning: I could not initialize audio!\n"
                      "The Simple DirectMedia error that occured was:\n"
                      "%s\n\n", SDL_GetError());
			  SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_WARNING, "Warning", err, window);
